...
| 12 | Reorder rebuilds cart | ⬜ (button placement pending) |
...
| 20 | Script tag in rich text stored sanitised | 🟡 (sanitise-on-save seam; DOMPurify to add) |
...
## Notable follow-ups
Full admin CRUD editors (product/order/content forms), DOMPurify sanitise-on-save,
reorder-to-cart, in-app checkout address step, React Email templates, and axe in
CI. The schema, RLS, server actions and page templates for all of these are in
place; the remaining work is wiring the admin write-forms and connecting live
credentials.
